...
class TextFieldAlertViewController: UIViewController {
    ...
    private var clickedDone: Binding<Bool>?
  ...
  ///   - isPresented: binding to be set to false when the alert is dismissed (`Done` button tapped)
    init(title: String, message: String?, text: Binding<String?>, isPresented: Binding<Bool>?, clickedDone: Binding<Bool>?) {
    self.alertTitle = title
    self.message = message
    self._text = text
    self.isPresented = isPresented
    self.clickedDone = clickedDone
  ...
  private func presentAlertController() {
    ...
    let action = UIAlertAction(title: "Done", style: .default) { [weak self] _ in
        ...
        self?.clickedDone?.wrappedValue = true
    }
    vc.addAction(action)
...
struct TextFieldAlert {
  ...
  var isPresented: Binding<Bool>? = nil
    var clickedDone: Binding<Bool>? = nil
...
extension TextFieldAlert: UIViewControllerRepresentable {
  ...
  func makeUIViewController(context: UIViewControllerRepresentableContext<TextFieldAlert>) -> UIViewControllerType {
    TextFieldAlertViewController(title: title, message: message, text: $text, isPresented: isPresented, clickedDone: clickedDone)
  }
